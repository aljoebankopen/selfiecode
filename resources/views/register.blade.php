...
<html lang="en">
  ...
  <form method="POST" action="{{ route('register.post') }}" style="width:300px">
    ...
    <div class="form-group">
        ...
        <input type="text" name="phone_number" id="phone_number" class="form-control @error('phone_number') is-invalid @enderror" value="{{ old('phone_number') }}" required autocomplete="phone_number">
        @error('phone_number')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
